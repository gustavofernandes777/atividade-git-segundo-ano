...
matriz.append(
    ["Inter de milao", 78, 11, 1, 1, 1, ["v", "v", "v", "v", "v"]]
)
for time in matriz:
...
matriz_ordenada = sorted(matriz[1:],key=lambda linha: linha[1],reverse=True)
...
for posicao, time in enumerate(matriz_ordenada):
    ...
    print(f"Últimos jogos: {time[6]}")
    print(f"Status: {time[7]}")
